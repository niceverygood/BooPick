...
> 마지막 갱신: 2026-05-06 / 진행 단계: **V2 Step 0 완료** (V1 정리 + 0008 마이그레이션 + 임차인용 랜딩 임시 hero)
>
> ⚠️ **2026-05-06 V1 → V2 가설 전환**. 메인 사용자 = 임차인. 자세한 변경은 [CLAUDE.md](../CLAUDE.md) 참조.
...
| 항목 | 값 |
|---|---|
| 정의 (V2) | 임차인을 모아서 가입 중개사에게 분배하는 인프라 |
| 회사 | Bottle Inc. (대표 한승수) |
| 메인 사용자 | **임차인 (B2C)** + 중개사 백오피스 |
| 핵심 베타 데이터 | 강남권 1선 중개사무소 매물 41,658건 (자체 정리, NDA 익명) |
| 목표 | 임차인 한 줄 검색 → 30초 매물 카드 → 카톡 컨택 → 등록 중개사 분배 |
| 진행 단계 | V2 Step 0 완료, Step 1 (임차인 진입점) 시작 예정 |
...
| 2026-05-06 | **V2 가설 전환** — 메인 사용자 중개사 → 임차인. 매물 풀 = 임차인 노출 슬롯 (default OFF). 가격 99k/299k/990k/성공보수형. 4주 4단계 로드맵 (Step 0~4). |
| 2026-05-06 | **Step 0 완료** — `/search` → `/agent/search` 이동, `/` 임차인용 임시 hero, 마이그레이션 0008 (tenants/tenant_searches/tenant_inquiries/subscription_plans + listings.tenant_pool_enabled + tenant_search_listings RPC). 데이터 출처 (A) 자체 정리 확인 — 학습 자료로 내부 사용 가능, 임차인 검색 노출 X. |
